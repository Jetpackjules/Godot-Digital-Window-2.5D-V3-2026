...
const PREBUILT_META := "_gdgs_prebuilt_raster_images"

## Returns {ok, core_texture, core_width, sh_texture, sh_width} or
## {ok=false, reason}.
static func build(resource, sh_half: bool = true) -> Dictionary:
	var built: Dictionary = resource.get_meta(PREBUILT_META, {}) if resource != null else {}
	if (
		not bool(built.get("ok", false))
		or int(built.get("point_count", -1)) != int(resource.get("point_count"))
	):
		built = build_images(resource, sh_half)
	elif resource.has_meta(PREBUILT_META):
		# The prepacked Images are a one-shot CPU handoff. Keeping them after
		# texture creation would retain a second full copy of the splat data.
		resource.remove_meta(PREBUILT_META)
