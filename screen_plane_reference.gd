...
func set_reference_mode(next_mode: int) -> void:
	var clamped_mode := clampi(next_mode, MODE_OFF, MODE_THIRDS_GRID)
	if mode == clamped_mode:
		return
	mode = clamped_mode

func set_reference_style(color: Color, thickness_ratio: float, minimum_thickness: float, depth_offset: float) -> void:
	var changed := false
...
func _add_vertical_bar(normalized_x: float) -> void:
	var thickness := _get_thickness_meters()
	var x := normalized_x * _bounds_size_meters.x * 0.5
...
func _add_horizontal_bar(normalized_y: float) -> void:
	var thickness := _get_thickness_meters()
	var y := normalized_y * _bounds_size_meters.y * 0.5
...
func _get_thickness_meters() -> float:
	return maxf(minimum_thickness_meters, _bounds_size_meters.y * thickness_ratio_of_bounds_height)
